#  ------------------------------------------------------------------------------------------
#  Our implementation of GPT-2 based model.
#  ------------------------------------------------------------------------------------------
import math
import copy
import torch
...
class GPT2Config(object):
    """
    This is the mGPT default configuration.
    """
    def __init__(
        self,
...
class GPT2LMModel(nn.Module):
    ...
    def forward(
        ...
        if lm_labels is not None:
            loss_fct = nn.CrossEntropyLoss(ignore_index=-100, reduce=False)
            if label_smooth > 0.0001:
                logprobs = torch.nn.functional.log_softmax(lm_logits.view(-1, lm_logits.size(-1)), dim=-1)
                nll_loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), lm_labels.view(-1))
